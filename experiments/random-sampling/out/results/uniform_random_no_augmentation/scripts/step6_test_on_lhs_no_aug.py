...
#I/O PATHS 
MODELS_DIR = Path("experiments/random-sampling/out/results/uniform_random_no_augmentation/trained models")
TEST_DATA_DIR = Path("experiments/lhs-sampling/data/split")
RESULTS_DIR= Path("experiments/random-sampling/out/results/uniform_random_no_augmentation/lhs_testing_no_aug")
PLOTS_DIR= Path("experiments/random-sampling/out/results/uniform_random_no_augmentation/lhs_testing_no_aug")



TRAIN_STRATEGY = 'UNIFORM_RANDOM'  
TEST_STRATEGY= 'LHS'  
AUGMENTATION= 0      
...
def build_replicate_dataframe(results_list, targets,
    ...
    if not rows:
        print("\n  ERROR: No rows built — results_list is empty")
        print(f"  Check that model files exist in: {Path(rows[0]['model_path']).parent if rows else 'unknown'}")
        return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    if not results_list:
        print(f"\n  FATAL: No model checkpoints found in {Path(args.models_dir).resolve()}")
        print("  Check that 'trained models' directory exists and contains best_balanced_mlp_model_*.pt files.")
        raise SystemExit(1)
