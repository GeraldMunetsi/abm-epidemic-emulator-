...
RESULTS_DIR= Path("experiments/random-sampling/out/results/uniform_random_no_augmentation/mcmc_testing_no_aug")
PLOTS_DIR= Path("experiments/random-sampling/out/results/uniform_random_no_augmentation/mcmc_testing_no_aug")
...
def plot_infected_only(results_list, targets, plots_dir, n_samples=8):
    ...
    axes = axes.flatten()
    fig.suptitle('UNIFORM RANDOM MODEL(NO AUGMENTATION) ON MCMC TEST SET (INFECTED (I) COMPARTMENT)',
        fontsize=22,
        fontweight='bold'
    )
