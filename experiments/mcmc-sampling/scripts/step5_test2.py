...
def load_replicate_model(model_path: Path, device: torch.device):
    ...
    # Fix knot count mismatch (predict_g_coeffs off-by-one)
    g_w = 'temporal_decoder.predict_g_coeffs.2.weight'
    g_b = 'temporal_decoder.predict_g_coeffs.2.bias'
    if g_w in state_dict:
        expected = config.get('n_knots', 18)
        actual   = state_dict[g_w].shape[0]
        if actual == expected - 1:
            state_dict[g_w] = torch.cat(
                [state_dict[g_w], torch.zeros(1, state_dict[g_w].shape[1])], dim=0)
            state_dict[g_b] = torch.cat(
                [state_dict[g_b], torch.zeros(1)], dim=0)
            print(f"  [COMPAT] Fixed g_coeffs knot mismatch ({actual} → {expected})")
...
def evaluate_all_replicates(models_dir, test_loader, device, n_timesteps):
    ...
    if not model_paths:
        raise FileNotFoundError(
            f"No best_balanced_mlp_model_*.pt files found in {models_dir}")
...
def compute_relative_mae_i(predictions, targets):
    ...
    mean_peak : float  — mean ground-truth peak I (for reporting)
    """
    ...
    return float(rel.mean()), float(rel.std(ddof=1) if len(rel) > 1 else 0.0), \
           rel, float(peak_per_sample.mean().item())
...
def save_results(results_list, stats_dict, output_dir):
    ...
    lines = [
        ...
        "=" * 70,
        "",
        f'"The 3-parameter SIR neural network emulator achieved a test set',
        f'MAE_I of {mae_i_mean:.0f} counts ({mae_i_ci[0]:.0f}–{mae_i_ci[1]:.0f},',
        f'95% CI, n={stats_dict["n_replicates"]} replicates), corresponding to',
        f'{rel_mean:.1f}% of the mean ground-truth peak infected count',
        f'({mean_peak:,.0f} individuals), with overall R² = {r2_mean:.4f} ±',
        ...
        "  · 95% CI: t-distribution (appropriate for small n)",
        "  · CV = std / mean × 100",
        "  · Relative MAE_I: per-sample (MAE_I_i / peak_I_i × 100), then averaged",
        "    This avoids dividing mean-by-mean (Jensen's inequality bias).",
        "    Samples with peak_I < 1 excluded from relative calculation.",
        "",
        "=" * 70,
    ]

    report = "\n".join(lines)
    (output_dir / 'FINAL_DISSERTATION_RESULTS.txt').write_text(report, encoding='utf-8')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Final test evaluation — 3-Parameter SIR emulator"
    )
    ...
    print("\n" + "=" * 70)
    print("STEP 5: FINAL TEST  —  3-PARAMETER SIR EMULATOR (τ, γ, ρ)")
    ...
    print(f"\n{'='*70}")
    print("TEST COMPLETE")
    print(f"  R²              : {stats_dict['R2']['mean']:.4f} "
          f"± {stats_dict['R2']['std']:.4f}")
    print(f"  Absolute MAE_I  : {stats_dict['MAE_I']['mean']:.2f} "
          f"± {stats_dict['MAE_I']['std']:.2f}")
