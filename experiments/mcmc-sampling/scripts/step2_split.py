...
def split_dataset(dataset,
    ...
    # Step 2: Random shuffle and assign to splits 
    ...
    # Step 3: Collect simulation indices for each split 
    ...
    # Step 4: Verify zero leakage 
    train_set = set(param_keys[i] for i in train_param_idx)
    val_set   = set(param_keys[i] for i in val_param_idx)
    test_set  = set(param_keys[i] for i in test_param_idx)

    leak_tr_te = train_set & test_set
    leak_tr_va = train_set & val_set
    leak_va_te = val_set   & test_set

    print("\nLeakage check")
    print(f"  Train ∩ Test : {len(leak_tr_te)}  (must be 0)")
    print(f"  Train ∩ Val  : {len(leak_tr_va)}  (must be 0)")
    print(f"  Val ∩ Test : {len(leak_va_te)}  (must be 0)")

    if any([leak_tr_te, leak_tr_va, leak_va_te]):
    ...
    # R₀ coverage report 
    print("\nR₀ coverage per split  (R₀ = tau/gamma × {:.3f})".format(RATIO))
...
def export_training_csv(split_data: dict, output_csv_path: Path) -> pd.DataFrame:
    ...
    R₀, epidemic summaries, and a near-threshold flag.
    """
    rows = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split SIR dataset by parameter set — leakage-free random split"
    )
    parser.add_argument('--input',type=str,default="epidemic_data_age_adaptive_sobol.pkl",
                        help="Filename inside RAW_DATA_DIR")
    parser.add_argument('--output',type=str,default=None,
                        help="Output pickle filename (default: <input>_split.pkl)")
    parser.add_argument('--output_csv',type=str,default=None,
                        help="Output CSV filename (default: <input>_train_params.csv)")
