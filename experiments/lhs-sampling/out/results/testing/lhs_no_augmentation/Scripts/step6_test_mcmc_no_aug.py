...
RESULTS_DIR= Path("experiments/lhs-sampling/out/results/testing/lhs_no_augmentation/mcmc_testing")
PLOTS_DIR= Path("experiments/lhs-sampling/out/results/testing/lhs_no_augmentation/mcmc_testing")
...
def plot_infected_only(results_list, targets, plots_dir, n_samples=8):
    ...
    axes = axes.flatten()
    fig.suptitle('LHS MODEL(NO AUGMENTATION) ON MCMC TEST SET (INFECTED (I) COMPARTMENT)',
        fontsize=14,
        fontweight='bold'
    )
