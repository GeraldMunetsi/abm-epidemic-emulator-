...
def split_dataset(dataset,
    ...
    # Verifying ratios sum to 1.0 otherwise raise a ValueError
    ...
    #  Step 1: Grouping simulations by parameter set 
    ...
    for name, p_idx in [("train", train_param_idx),
        ...
        r0s = np.array([(param_keys[i][0] / param_keys[i][1]) * RATIO
                         for i in p_idx])
        sub   = (r0s < 0.1).sum()
        thr   = ((r0s >= 0.1) & (r0s <= 2)).sum()
        sup   = (r0s > 2).sum()
        n     = len(r0s)
        print(f"  {name:5s}  R0<0.5: {sub:4d} ({100*sub/n:.1f}%)  "
...
def export_training_csv(split_data: dict, output_csv_path: Path) -> pd.DataFrame:
    ...
    if 'R0' in df.columns:
        total = len(df)
        print(f"R₀ < 0.1 : {(df['R0'] < 0.1).sum():5d}  ({100*(df['R0']<0.1).mean():.1f}%)")
        print(f"0.1 ≤ R₀ ≤ 2 : {((df['R0']>=0.1)&(df['R0']<=2)).sum():5d}  "
              f"({100*((df['R0']>=0.1)&(df['R0']<=2)).mean():.1f}%)")
        print(f"R₀ > 2: {(df['R0'] > 2).sum():5d}  ({100*(df['R0']>2).mean():.1f}%)")
