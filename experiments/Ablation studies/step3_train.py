...
from utils import create_dataloaders, compute_metrics, compute_ablation_metrics, get_device, EarlyStopping

#I/O Paths
DATA_DIR=Path("experiments/mcmc-sampling/data/augmented")
MODEL_DIR=Path("experiments/Ablation studies/out/trained-models")
...
def compute_balanced_loss(predictions, targets, device, weight_mode='balanced',
    ...
    # Soft conservation penalty for the no_conservation ablation
    if condition == 'no_conservation':
        cons_residual = (S_pred + I_pred + R_pred - N) / N
        cons_penalty  = cons_residual.pow(2).mean()
        total_loss    = total_loss + 10.0 * cons_penalty
...
def train_epoch_balanced(model, train_loader, optimizer, device, n_timesteps,
                         weight_mode='modest', condition='full'):
    ...
    for batch in train_loader:
        ...
        loss, loss_S, loss_I, loss_R = compute_balanced_loss(
            predictions, targets, device, weight_mode, condition
        )
...
def validate_balanced(model, val_loader, device, n_timesteps, weight_mode='modest',
    ...
    targets= torch.cat(all_targets,dim=0)
    metrics= compute_metrics(predictions, targets)
    metrics['rel_mae_i'] = compute_ablation_metrics(predictions, targets)['rel_mae_i']

    return total_loss/len(val_loader),metrics

# SINGLE REPLICATE TRAINING
def train_single_replicate(replicate_id, seed, config, dataloaders, output_dir,
                           condition='full', weight_mode='modest', verbose=True):
    """
    ...
    if verbose:
        print(f"REPLICATE {replicate_id}  (seed={seed})  condition={condition}")
        print(f"Parameters: tau , gamma , rho ")
        print(f"Saving to:{model_filename}")

    # Build model for this ablation condition
    model = create_ablation_model(condition, config).to(device)
    ...
    history = {
        ...
        'train_mae_i': [], 'val_mae_i': [], 'val_rel_mae_i': [],
    ...
    for epoch in range(config['epochs']):
        train_loss, train_metrics = train_epoch_balanced(
            model, train_loader, optimizer, device, n_timesteps, weight_mode, condition
        )
        val_loss, val_metrics = validate_balanced(
            model, val_loader, device, n_timesteps, weight_mode, condition
        )
        #scheduler.step()
        ...
        history['train_mae'].append(train_metrics['MAE']) #global training MAE
        ...
        history['val_rel_mae_i'].append(val_metrics['rel_mae_i'])
        history['train_mae_r'].append(train_metrics['MAE_R'])
        history['val_mae_r'].append(val_metrics['MAE_R'])
        history['train_r2'].append(train_metrics['R2'])
        history['val_r2'].append(val_metrics['R2'])
        history['train_r2_I'].append(train_metrics['R2_I'])
        history['val_r2_I'].append(val_metrics['R2_I'])

        if verbose and ((epoch + 1) % 2 == 0 or epoch == 0 or epoch == config['epochs'] - 1):
            print(f"Epoch {epoch+1:3d}/{config['epochs']}: "
                  f"Val R²_I={val_metrics['R2_I']:.4f}, "
                  f"MAE_I={val_metrics['MAE_I']:.2f}")

        # Save best checkpoint
        if val_metrics['R2'] > best_val_r2:
            best_val_r2=val_metrics['R2_I']
    ...
    if verbose:
        print(f"Best epoch : {best_epoch}  |  Val R²_I: {best_val_r2:.4f}  |  "
...
def train_multiple_replicates(
    n_replicates, seeds, config, dataloaders, output_dir,
    condition='full', weight_mode='modest'
):
    """
    ...
    print(f"TRAINING {n_replicates} REPLICATES  [condition={condition}]")
...
def compute_replicate_statistics(all_results):
    ...
    for metric in metrics:
        ...
        ci= stats.t.interval(0.95, n - 1, loc=mean, scale=sem) if n > 1 else (mean, mean) #t-distribution
...
def create_summary_report(all_results, stats_dict, output_dir, weight_mode):
    ...
    lines += [
        ...
        f"{'ID':>3} {'Seed':>6} {'Epoch':>6} {'R²_I':>8} {'MAE_I':>8} "
...
if __name__ == "__main__":
    ...
    # Each condition writes to its own sub-directory so runs don't overwrite each other
    ...
    write_training_timing_log(
        ...
        overall_time,
        config,
        output_dir / "training_timing_log.txt",
    )
    ...
    with open(output_dir / 'replicates_summary.json', 'w') as f:
        json.dump({'results': all_results, 'statistics': stats_dict,
                   ...
                   'weight_mode': args.weight_mode}, f, indent=2)
    ...
    print(f"\n Validate: python step4_validate.py --models_dir {output_dir} --condition {args.condition}")
    print(f"  Test   : python step5_test2.py --models_dir {output_dir} --condition {args.condition}")
