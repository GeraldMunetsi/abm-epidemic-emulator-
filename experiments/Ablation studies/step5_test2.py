...
from step0_model import create_ablation_model, ABLATION_CONDITIONS
...
# I/O PATHS
# All defaults live under this experiment's own out/ tree, namespaced by
# --condition, so they never collide with the mcmc-sampling experiment's
# models/results or with the shared Regression/data master dataset.
ABLATION_MODELS_ROOT  = Path("experiments/Ablation studies/out/trained-models")
TEST_DATA_DIR         = Path("experiments/mcmc-sampling/data/split")
ABLATION_RESULTS_ROOT = Path("experiments/Ablation studies/out/results/testing")
ABLATION_PLOTS_ROOT   = Path("experiments/Ablation studies/out/plots/testing")
...
# MODEL LOADING
def load_replicate_model(model_path: Path, device: torch.device, condition: str):
    """
    Load a single replicate checkpoint for the given ablation condition.
    """
    checkpoint = torch.load(model_path, map_location=device, weights_only=False)

    ckpt_condition = checkpoint.get('condition')
    if ckpt_condition is not None and ckpt_condition != condition:
        raise ValueError(
            f"Condition mismatch: checkpoint {model_path.name} was trained as "
            f"'{ckpt_condition}' but --condition={condition!r} was requested."
        )
    ...
    model = create_ablation_model(condition, config)
...
def evaluate_all_replicates(models_dir, test_loader, device, n_timesteps, condition):
    ...
    print(f"\n{'-'*70}")
    print(f"TEST EVALUATION·{len(model_paths)} REPLICATE(S)  [condition={condition}]")
    results_list = []
    shared_targets= None
    shared_params= None
    ...
    for idx, path in enumerate(model_paths, 1):
        ...
        model, checkpoint = load_replicate_model(path, device, condition)
...
def compute_aggregate_statistics(results_list, targets):
    ...
    rel_std = rel_arr.std(ddof=1) if n > 1 else 0.
    rel_sem = rel_std / np.sqrt(n) if n > 1 else 0.
...
# VISUALISATION
def plot_all_compartments(results_list, targets, plots_dir, params=None, n_samples=8):
    ...
    for row, idx in enumerate(indices):
        ...
        if params is not None:
            tau, gam, rho = params[idx,0].item(), params[idx,1].item(), params[idx,2].item()
            r0 = tau / gam * 58
            tau_str = f"τ={tau:.3f}, γ={gam:.3f}, ρ={rho:.3f}, R₀={r0:.2f}"
        else:
            tau_str = ""
        for col in range(3):
            ...
            ax.text(0.02, 0.97, label, transform=ax.transAxes, fontsize=10,
                    fontweight='bold', va='top', ha='left')
            if row == 0:
                ax.set_title(compartments[col], fontsize=11, fontweight='bold')
            # Parameter annotation on leftmost column
            if col == 0 and tau_str:
                ax.annotate(tau_str, xy=(0.5, 1.02), xycoords='axes fraction',
...
def plot_infected_only(results_list, targets, plots_dir, params=None, n_samples=8):
    plots_dir = Path(plots_dir)
    ...
    for panel_idx, idx in enumerate(indices):
        ax = axes[panel_idx]
        target = targets_np[idx]
        ax.plot(target[:, 1], 'o', color='steelblue', alpha=0.7, markersize=4,
                label='Ground Truth', zorder=10)

        for rep_i, result in enumerate(results_list):
            pred = result['predictions'][idx].detach().cpu().numpy()
            ax.plot(pred[:, 1], '-', color=pred_colors[rep_i], linewidth=1.5,
                    alpha=0.7, label=f"M{result['replicate_id']}")
        ...
        if params is not None:
            tau, gam, rho = params[idx,0].item(), params[idx,1].item(), params[idx,2].item()
            r0 = tau / gam * 58
            param_str = f"τ={tau:.3f}  γ={gam:.3f}  ρ={rho:.3f}  R₀={r0:.2f}"
            ax.set_title(param_str, fontsize=10, color='black', pad=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--condition', type=str, default=CONDITION,
                         ...
                         help='Ablation condition whose checkpoints to test')
    parser.add_argument('--models_dir',type=str, default=None,
                         help='Defaults to experiments/Ablation studies/out/trained-models/<condition>')
    parser.add_argument('--data',type=str,default=str(TEST_DATA_DIR /'abm-data_split.pkl'))
    parser.add_argument('--output_dir', type=str, default=None,
                         help='Defaults to experiments/Ablation studies/out/results/testing/<condition>/mcmc_test')
    parser.add_argument('--plots_dir',type=str, default=None,
                         help='Defaults to experiments/Ablation studies/out/plots/testing/<condition>/mcmc_test')
    ...
    models_dir = Path(args.models_dir) if args.models_dir else ABLATION_MODELS_ROOT / CONDITION
    results_dir = Path(args.output_dir) if args.output_dir else ABLATION_RESULTS_ROOT / CONDITION / 'mcmc_test'
    plots_dir   = Path(args.plots_dir)  if args.plots_dir  else ABLATION_PLOTS_ROOT / CONDITION / 'mcmc_test'
    results_dir.mkdir(parents=True, exist_ok=True)
    plots_dir.mkdir(parents=True, exist_ok=True)

    if args.build_master:
        master = build_master_dataframe(ABLATION_RESULTS_ROOT)
        mp = ABLATION_RESULTS_ROOT / 'master_replicate_results.csv'
        master.to_csv(mp, index=False)
        print(f"\nSaved master: {mp.resolve()}")
        raise SystemExit(0)

    print("\n"+"-"*70)
    print(f"STEP 5: TEST : condition={CONDITION}  {TRAIN_STRATEGY}:{TEST_STRATEGY}  aug={AUGMENTATION}")
    ...
    results_list, targets, params = evaluate_all_replicates(
        models_dir, test_loader, device, n_timesteps, CONDITION
    )
    ...
    f"test_replicate_results_{CONDITION}_"
    ...
    # Plots
    plot_all_compartments(results_list, targets, plots_dir, params=params, n_samples=args.n_samples)
    plot_infected_only(results_list, targets, plots_dir, params=params, n_samples=args.n_samples)
