...
from utils import create_dataloaders, compute_metrics, compute_ablation_metrics, compute_zone_r2i, get_device, PARAM_MINS, PARAM_MAXS

n_timepoints = 250
N=100000
knots=8

DATA_DIR=Path("experiments/Ablation studies/out/trained-models")
SPLIT_DATA_DIR=Path("experiments/mcmc-sampling/data/split")
MODEL_DIR=Path("experiments/Ablation studies/out/trained-models")
PLOTS_DIR=Path("experiments/Ablation studies/out/plots")
...
# MODEL LOADING
def load_model(model_path, device, condition='full'):
    """
    ...
    Args:
        ...
        device     : torch.device
        condition  : ablation condition used when the model was trained
    ...
    # Condition stored in checkpoint takes precedence over the CLI argument
    condition = checkpoint.get('condition', condition)
...
def evaluate_model(model, val_loader, device, n_timesteps):
    ...
    metrics.update(compute_zone_r2i(predictions, targets, params))
    return predictions, targets, params, metrics


def evaluate_all_replicates(models_dir, val_loader, device, n_timesteps, condition='full'):
    """
    ...
    """
    condition_dir = Path(models_dir) / condition
    model_paths = find_replicate_models(condition_dir)

    print(f"\n{'-'*70}")
    print(f"EVALUATING {len(model_paths)} REPLICATES")
    print(f"Models directory : {condition_dir}")
    ...
    for idx, model_path in enumerate(model_paths, 1):
        ...
        model, checkpoint = load_model(model_path, device, condition)
        ...
        ablation_metrics = compute_ablation_metrics(predictions, targets_rep)
        result = {
            ...
            'metrics'        : {**metrics, **ablation_metrics},
        }

        results_list.append(result)
        print(f"R² = {metrics['R2']:.4f} | MAE_I = {metrics['MAE_I']:.2f} | "
              f"Rel-MAE_I = {ablation_metrics['rel_mae_i']:.2f}% | "
              f"Cons. err = {ablation_metrics['conservation_error']:.4f}%\n")
...
def compute_aggregate_statistics(results_list):
    ...
    metric_names = ['MAE', 'RMSE', 'R2', 'MAE_S', 'MAE_I', 'MAE_R',
                    'R2_I', 'R2_I_low', 'R2_I_med', 'R2_I_high',
                    'rel_mae_i', 'conservation_error']
    stats_dict   = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Output directory for results (JSON, CSV, report); defaults to models_dir')
    ...
    parser.add_argument('--condition', type=str, default='full',
                        ...
                        help='Ablation condition (must match the trained models)')
    args = parser.parse_args()

    # Resolve all paths
    models_dir=Path(args.models_dir)
    data_path=Path(args.data)
    results_dir=Path(args.output_dir) if args.output_dir is not None else models_dir / args.condition
    ...
    results_list, targets, params = evaluate_all_replicates(
        models_dir, val_loader, device, n_timesteps, condition=args.condition
    )

    if not results_list:
        raise RuntimeError(
            f"No replicate models found for condition '{args.condition}' in:\n"
            f"  {(models_dir / args.condition).resolve()}\n"
            f"Expected files matching: best_balanced_mlp_model_*.pt"
        )
    ...
    #  Visualisations
    plot_training_curves(results_list, models_dir / args.condition, plots_dir)
