...
class TemporalDecoder(nn.Module):
    ...
    First spline coefficient is pinned to 8.0 so sigmoid(8) ≈ 1 → y(0) ≈ 1.
...
# 6. Testing if conservation, non-negativity, and initial conditions are satisfied 
...
# ABLATION 1: No RFF — raw params_norm (dim=3) fed directly to fusion MLP 
#Does RFF  mapping improve generalization?
class NoRFFEmulator(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        n_params= config.get('n_params', 3)
        fusion_hidden= config.get('fusion_hidden', 128)
        ...
        self.fusion = nn.Sequential(
            nn.Linear(n_params, fusion_hidden),
            nn.LayerNorm(fusion_hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(fusion_hidden, latent_dim),
        ...
        self.temporal_decoder = TemporalDecoder(
            latent_dim=latent_dim, n_knots=n_knots,
            total_population=total_population, hidden_dim=decoder_hidden,
        )
        self.n_timepoints = timepoints

    #Skipping RFF entirely
    def forward(self, data, n_timesteps=None):
        z = self.fusion(data.params_norm)  #using raw params_norm directly
        S_pred, I_pred, R_pred = self.temporal_decoder(z, data.rho_raw)
        return torch.stack([S_pred, I_pred, R_pred], dim=2)

    def get_component_params(self):
        def count(m): return sum(p.numel() for p in m.parameters() if p.requires_grad)
        return {
            'rff_trainable': 0, 'rff_frozen': 0,
            'fusion': count(self.fusion),
            'temporal_decoder': count(self.temporal_decoder),
            'total': count(self),
        }
...
# ABLATION 2: No B-spline — three independent sigmoid MLP heads (conservation violated)
class MLPOnlyDecoder(nn.Module):
    """Three independent MLP heads for S, I, R — no B-spline"""
    def __init__(self, latent_dim: int, n_timepoints: int = timepoints,
                 total_population: int = N, hidden_dim: int = 64):
        super().__init__()
        self.N = float(total_population)
        ...
        def _head():
            return nn.Sequential(
                nn.Linear(latent_dim, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, n_timepoints),
            )
        self.head_S = _head()
        self.head_I = _head()
        self.head_R = _head()
    ...
    def forward(self, z: torch.Tensor, rho_raw: torch.Tensor) -> tuple:
        S = torch.sigmoid(self.head_S(z)) * self.N   # ∈ [0, N], no structural constraint
        I = torch.sigmoid(self.head_I(z)) * self.N
        R = torch.sigmoid(self.head_R(z)) * self.N
        return S, I, R


class NoSplineEmulator(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        n_params = config.get('n_params', 3)
        n_fourier= config.get('n_fourier', 64)
        ...
        rff_out = 2 * n_fourier
        self.rff = StandardRFF(n_params=n_params, n_fourier=n_fourier, sigma=sigma)
        self.fusion = nn.Sequential(
            nn.Linear(rff_out, fusion_hidden),
            nn.LayerNorm(fusion_hidden),
            nn.ReLU(),
        ...
        self.temporal_decoder = MLPOnlyDecoder(
            latent_dim=latent_dim, n_timepoints=timepoints,
            total_population=total_population, hidden_dim=decoder_hidden,
        )
        self.n_timepoints = timepoints

    def forward(self, data, n_timesteps=None):
        phi = self.rff(data.params_norm)
        z = self.fusion(phi)
        S_pred, I_pred, R_pred = self.temporal_decoder(z, data.rho_raw)
        return torch.stack([S_pred, I_pred, R_pred], dim=2)
    ...
    def get_component_params(self):
        def count(m): return sum(p.numel() for p in m.parameters() if p.requires_grad)
        rff_frozen = sum(b.numel() for b in self.rff.buffers())
        return {
            'rff_trainable': count(self.rff), 'rff_frozen': rff_frozen,
            'fusion': count(self.fusion),
            'temporal_decoder': count(self.temporal_decoder),
            'total': count(self),
        }
...
# ABLATION 3: No conservation — independent B-splines for S, I, R (soft penalty in loss)
class NoConservationDecoder(nn.Module):
    """
    S: monotone-decreasing B-spline (structural).
    R: monotone-increasing B-spline (structural).
    I: free positive B-spline — independent of S and R.
    S + I + R ≠ N; conservation enforced only by a soft loss penalty.
    """
    def __init__(self, latent_dim: int, n_knots: int = knots,
                 total_population: int = N, hidden_dim: int = 64):
        super().__init__()
        self.N = float(total_population)
        self.n_knots = n_knots
        ...
        self.spline_S = BSplineLayer(n_knots, timepoints)
        self.spline_I = BSplineLayer(n_knots, timepoints)
        self.spline_R = BSplineLayer(n_knots, timepoints)
    ...
    def forward(self, z: torch.Tensor, rho_raw: torch.Tensor) -> tuple:
        ...
        # S: monotone decreasing from S_0 = (1-rho)*N
        S_0     = ((1.0 - rho_raw) * self.N).unsqueeze(1)
        ret_raw = self.predict_S_retention(z)
        ret     = torch.sigmoid(ret_raw)
        ones_s  = torch.ones(batch_size, 1, device=device)
        S_coeffs = S_0 * torch.cumprod(torch.cat([ones_s, ret], dim=1), dim=1)
        S_pred   = self.spline_S(S_coeffs)

        # I: free positive B-spline — no relationship to S or R
        I_coeffs = F.softplus(self.predict_I_coeffs(z)) * self.N * 0.05
        I_pred   = self.spline_I(I_coeffs)

        # R: monotone increasing from 0
        r_raw   = self.predict_R_rates(z)
        r       = torch.sigmoid(r_raw)
        ones_r  = torch.ones(batch_size, 1, device=device)
        cum_r   = torch.cumprod(torch.cat([ones_r, r], dim=1), dim=1)
        R_coeffs = (1.0 - cum_r) * self.N
        R_pred   = self.spline_R(R_coeffs)

        return S_pred, I_pred, R_pred


class NoConservationEmulator(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        n_params         = config.get('n_params', 3)
        n_fourier        = config.get('n_fourier', 64)
        ...
        self.temporal_decoder = NoConservationDecoder(
            latent_dim=latent_dim, n_knots=n_knots,
            total_population=total_population, hidden_dim=decoder_hidden,
        )
        self.n_timepoints = timepoints
...
ABLATION_CONDITIONS = {
    'full'           : HybridSIREmulator,
    'no_rff'         : NoRFFEmulator,
    'no_spline'      : NoSplineEmulator,
    'no_conservation': NoConservationEmulator,
}


def create_ablation_model(condition: str, config: dict) -> nn.Module:
    """
    Build an ablation variant of the SIR emulator.

    Args:
        condition : 'full', 'no_rff', 'no_spline
    Returns:
        model : nn.Module
    """
    if condition not in ABLATION_CONDITIONS:
        raise ValueError(
            ...
            f"Choose from: {list(ABLATION_CONDITIONS)}"
        )
    return ABLATION_CONDITIONS[condition](config)
