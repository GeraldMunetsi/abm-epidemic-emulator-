...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Step 1 — Random Sampling data generation"
    )
    parser.add_argument('--experiment', type=str, default='random',
                        choices=['random', 'lhs', 'mcmc'],
                        help="Experiment to generate data for (default: random).")
    ...
    print(f"  Next step: run step2 split notebook, then:")
    print(f"    python src/step3_train.py --experiment {args.experiment}")



random_sampling_data=pd.read_csv('epidemic_data_age_adaptive_sobol.csv')
