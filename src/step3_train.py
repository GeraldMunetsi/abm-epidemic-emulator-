...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment',   type=str,   default=None,
                        ...
                        help='Which sampling experiment to train. '
                             'Auto-resolves all input/output paths.')
    ...
    parser.add_argument('--output_dir',   type=str,   default='replicates_outputs',
                        help='Fallback model output dir (ignored when --experiment is set).')
    ...
    if args.experiment:
        ...
        results_dir = paths['results_val']          # training summary → validation folder
        data_file   = get_training_data(args.experiment)  # augmented > split > raw
        args.output_dir = str(models_dir)
        ...
        if args.input is None:
            args.input = 'epidemic_data_age_adaptive_sobol_split_augmented.pkl'
    ...
    print(" SIR EMULATOR — REPLICATE TRAINING  (τ, γ, ρ)")
    ...
    dataloaders = create_dataloaders(str(data_file), batch_size=config['batch_size'])
    ...
    with open(results_dir / 'replicates_summary.json', 'w') as f:
        json.dump({'results': all_results, 'statistics': stats_dict,
                   'config': config, 'weight_mode': args.weight_mode}, f, indent=2)

    create_summary_report(all_results, stats_dict, results_dir, args.weight_mode)
    plot_replicates_comparison(all_results, all_histories, plots_dir)

    df = pd.DataFrame(all_results)
    df.to_csv(results_dir / 'replicates_results.csv', index=False)
    print(f"Saved: {results_dir / 'replicates_results.csv'}")
